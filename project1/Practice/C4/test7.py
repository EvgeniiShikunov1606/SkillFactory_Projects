G = {"Адмиралтейская": ["Садовая"],
     "Садовая": ["Адмиралтейская", "Звенигородская", "Спасская", "Сенная площадь"],
...
D = {k: 100 for k in G1.keys()}  # расстояния
start_k = 'Адмиралтейская'  # стартовая вершина
D[start_k] = 0  # расстояние от неё до самой себя равно нулю
U = {k: False for k in G1.keys()}  # флаги просмотра вершин
P = {k: None for k in G1.keys()}  # предки

for _ in range(len(D)):
    # выбираем среди непросмотренных наименьшее по расстоянию
    min_k = min([k for k in U.keys() if not U[k]], key=lambda x: D[x])

    for v in G1[min_k].keys():  # проходимся по всем смежным вершинам
        if D[v] > D[min_k] + G1[min_k][v]:  # если расстояние от текущей вершины меньше
            D[v] = D[min_k] + G1[min_k][v]  # то фиксируем его
